...
(function () {
  ...
  // health.html#whoop_access=...&whoop_refresh=.... The same applies to
  // Google Calendar's own token handback, which lands on
  // dashboard.html#gcal_access=...&gcal_refresh=.... Either way the
  ...
  // ever reads them.
  if (/[?&]code=/.test(window.location.search) || /whoop_access/.test(window.location.hash) || /gcal_access/.test(window.location.hash)) return;
