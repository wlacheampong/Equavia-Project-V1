...
// Read-only counterpart to fetchMergeUpsertArray -- see that function's
// comment for why Vitality-bridge domains need their own array-shaped path.
export async function readArrayDomain(appKey) {
  const res = await fetch(
    SUPABASE_URL + '/rest/v1/app_state?key=eq.' + encodeURIComponent(appKey) + '&select=data',
    { headers: { apikey: SUPABASE_KEY, Authorization: 'Bearer ' + SUPABASE_KEY } }
  );
...
// Vitality-bridge domains (notes.html/train.html's window.Vitality.save/load,
// e.g. appKey='vitality_notes') store the raw payload directly as `data` --
// an array, not a {localStorageKey: value} map like every sync.js-driven
// domain above. Same fetch-merge-upsert safety property, just a different
// default shape (empty array, not empty object) and no key indirection.
export async function fetchMergeUpsertArray(appKey, mutateFn) {
  const res = await fetch(
    SUPABASE_URL + '/rest/v1/app_state?key=eq.' + encodeURIComponent(appKey) + '&select=data',
    { headers: { apikey: SUPABASE_KEY, Authorization: 'Bearer ' + SUPABASE_KEY } }
  );
  ...
  const result = await mutateFn(arr);
  await upsertRow(appKey, arr);
  return result;
}
