...
import { z } from 'zod';
import { readDomain, fetchMergeUpsert, readArrayDomain, fetchMergeUpsertArray } from './mcp-data.js';
...
// Matches notes.html's slugifyId() exactly -- a note's id is also its
// Obsidian sync filename (AppNotes/<id>.md), so a readable slug beats an
// opaque random string, same reasoning notes.html's own comment gives.
function slugifyId(title, existingIds) {
  const base = String(title || '').toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60);
  if (!base) return genId('note-');
...
export const TOOLS = [
  ...
  // ---------- Notes (notes.html, via the Vitality bridge -- app_state row
  // key='vitality_notes', data stored as a raw array, not a key-map) ----------
  {
    name: 'get_notes',
    description: 'Get recent notes (most recently updated first).',
    inputSchema: { limit: z.number().optional().describe('Max notes to return, default 20.') },
    handler: async ({ limit }) => {
      const notes = await readArrayDomain('vitality_notes');
      const sorted = notes.slice().sort((a, b) => (b.updatedAt || '').localeCompare(a.updatedAt || ''));
      const capped = sorted.slice(0, limit && limit > 0 ? limit : 20);
      return ok({ notes: capped.map((n) => ({ id: n.id, title: n.title, body: n.body, updatedAt: n.updatedAt })) });
    },
  },
  {
    name: 'add_note',
    description: "Create a new note (e.g. to save a conversation or a thought). Lands in Equavia's Notes tile immediately, cross-device -- but only reaches the actual Obsidian vault the next time Notes is opened on the device where Obsidian's Local REST API plugin is reachable, same as any note added by hand.",
    inputSchema: { title: z.string().optional(), body: z.string() },
    handler: async ({ title, body }) => {
      let created;
      await fetchMergeUpsertArray('vitality_notes', (notes) => {
        const existingIds = new Set(notes.map((n) => n.id));
        const id = slugifyId(title, existingIds);
        const now = new Date().toISOString();
        created = { id, title: title || '', body, images: [], pinned: false, createdAt: now, updatedAt: now, obsidian: { path: null, lastSyncedAt: null, remoteMtime: null } };
        notes.push(created);
      });
      return ok({ added: { id: created.id, title: created.title } });
    },
  },
];
